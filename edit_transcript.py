...
Production v3 (Anti-Hallucination): Добавляет защиту от галлюцинаций кода.

Изменения v3:
- Явный запрет на генерацию кода из памяти модели
- Инструкция использовать `[Пример кода демонстрируется на экране]` для непрочитанного кода
- Решает проблему "чрезмерной старательности" модели (оставшиеся 3.1% аномалий)
...
Результаты тестирования v2 (65 глав):
- Аномалии снижены с 9.2% до 3.1%
- 96.9% глав с качественным результатом
- Средняя компрессия: 52.65%

Ожидаемые результаты v3:
- Устранение оставшихся 3.1% аномалий (галлюцинации кода)
- ~99%+ качественных результатов
"""
...
def build_prompt_balanced_v2(chapter_title: str, chapter_text: str) -> str:
    ...
    Production v3 (Anti-Hallucination): Добавляет защиту от галлюцинаций кода.
    - v3: Явный запрет на генерацию кода из памяти модели
    - v2: Решает проблему преждевременной остановки на плотном техническом контенте
    """
...
**ВАЖНО - ЗАПРЕТ НА ГАЛЛЮЦИНАЦИИ КОДА:**
ЕСЛИ лектор упоминает код или показывает его ("вот так в PyTorch...", "здесь мы используем..."),
но НЕ зачитывает его полностью — НЕ генерируй код сам из своих знаний!
Вместо этого напиши: `[Пример кода демонстрируется на экране]` или опиши логику словами.
Используй ТОЛЬКО тот код, который явно прочитан вслух в транскрипте.
...
def edit_chapter_with_llm(
    ...
    """Редактирует главу через LLM (Production v3: Anti-Hallucination)."""
