"""Production-тест финальной конфигурации на всех 4 днях.
...
КОНФИГУРАЦИЯ:
- Модель: qwen3:32b
- Промпт: Balanced (редактор Хабра)
...
# === Balanced Prompt (финальный) ===

def build_prompt_balanced(chapter_title: str, chapter_text: str) -> str:
    """Промпт для сбалансированной модели (редактор Хабра)."""
...
def process_chapter(chapter: dict, paragraphs: dict, day_name: str, chapter_idx: int) -> dict:
    ...
    prompt = build_prompt_balanced(chapter_title, chapter_text)
...
def main():
    """Основная функция."""
    print("=== PRODUCTION-ТЕСТ: Qwen3:32b + Balanced (Final) ===")
    ...
    for day_info in DAYS:
        ...
        for idx, chapter in enumerate(chapters):
            ...
            if result["success"]:
                output_file = Path(f"production_output/{day_info['name']}_chapter_{idx+1}.md")
    ...
    report_file = Path(f"production_output/production_report_{timestamp}.json")
    ...
    # Итоговая статистика
    print(f"\n{'='*80}")
    print("[PRODUCTION] ИТОГОВАЯ СТАТИСТИКА")
    print(f"{'='*80}\n")
    ...
    print(f"[FILES] Отредактированные главы: production_output/")
    print("\n=== Production-тест завершён! ===")
