# genai-readme-step1.md — Stage 1: Cleaning & Packing

## Цель
Добавить независимый шаг пост‑обработки `postprocess.py`, который превращает первичные сегменты в абзацы и сохраняет их с таймкодами (если доступны). Не менять интерфейс `transcribe_v2.py`.

## Предпосылки
- В `TRANSCRIBE_OUT` (или `./transcribe`) уже лежат результаты транскрибации: `*.txt` и, по возможности, `*.srt / *.jsonl`.

## План работ

### 1) Создать файл `postprocess.py` в корне
Требования:
- только стандартная библиотека (опционально `python-dotenv`, если уже в проекте);
- вход: предпочесть JSONL → иначе SRT → иначе TXT (без таймкодов);
- вывод: `*_clean.md`, `*_packed.jsonl`, `*_clean.txt` (если включено), `*_postprocess_report.json`.

**Референс‑реализация (вставить полностью):**
```python
...
def strip_fillers_edge(s: str, fillers: list[str]) -> str:
    ...
    for _ in range(2):
        for token in fillers:
            s = re.sub(rf"^(?:{re.escape(token)})[,\s]+", "", s, flags=re.IGNORECASE)
            s = re.sub(rf"[,\s]+(?:{re.escape(token)})\.?$", "", s, flags=re.IGNORECASE)
    return s
...
def main() -> None:
    ...
    cfg = {
        ...
        "fillers_ru": [s.strip() for s in getenv_str("POST_FILLERS_RU","ну,как бы,то есть,ээ,эээ,короче,значит,в общем").split(",") if s.strip()],
        "fillers_en": [s.strip() for s in getenv_str("POST_FILLERS_EN","uh,um,like,you know,so,kinda,sort of").split(",") if s.strip()],
    }

    # собрать уникальные stem'ы
    stems = set()
    ...
    for stem in sorted(stems):
        src_jsonl, src_srt, src_txt = stem.with_suffix(".jsonl"), stem.with_suffix(".srt"), stem.with_suffix(".txt")
        ...
        if src_jsonl.exists():
            ...
            logger.info("Skip %s (no inputs)", stem.name); continue
        ...
        packed_path  = stem.parent / (stem.name + "_packed.jsonl")
        clean_md_path= stem.parent / (stem.name + "_clean.md")
        clean_txt_path=stem.parent / (stem.name + "_clean.txt")
        report_path  = stem.parent / (stem.name + "_postprocess_report.json")
        ...
        logger.info("Processed %s → %s / %s", stem.name, clean_md_path.name, packed_path.name)
...
- В `README.md` — добавить пункт «Postprocess: `python postprocess.py` → `*_clean.md`, `*_packed.jsonl`».
- В `QUICK_START.md` — в «Получите результаты» упомянуть новые артефакты.

### 3) Добавить базовые тесты (`tests/test_postprocess.py`)
Скелет:
```python
from pathlib import Path
...
### 4) Прогон
```bash
python transcribe_v2.py   # при необходимости
python postprocess.py
pytest -q                 # если добавлены тесты
```

### 5) Коммиты/PR
- `feat(postprocess): cleaning & packing with jsonl/srt/txt fallbacks`
- `docs: mention postprocess step and outputs`
```

## DoD (Definition of Done)
- Создаются `*_clean.md`, `*_packed.jsonl` и (если включено) отчёт.
- При наличии таймкодов абзацы содержат корректные `start/end`.
- Текст в `*_clean.md` без «привет/слышно‑видно» и лишних повторов.
- Ничего не ломает существующий запуск транскрибации.
